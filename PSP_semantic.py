...
class PSPDataset(Dataset):
    def __init__(self, batch_size, name, feature_set):  # name = train/dev
        path = '/new_temp/fsb/fsb_PSP/'
        self.edge_index = torch.load(path + 'edge_index.pt')
        self.edge_type = torch.load(path + 'edge_type.pt')
        self.node_features = torch.load(path + feature_set).float().squeeze(1)
        ...
        self.name = name
        if self.name == 'train':
            self.length = int(846 / self.batch_size)
...
class PSPDetector(pl.LightningModule):
    def __init__(self, in_channels, out_channels, dropout, batch_size, num_relations, negative_sample, negative_sample_weight):
        ...
        self.linear_liberal = nn.Linear(out_channels, 5)
        self.linear_conservative = nn.Linear(out_channels, 5)
        ...
        torch.nn.init.kaiming_uniform(self.linear_liberal.weight, nonlinearity='relu')
        torch.nn.init.kaiming_uniform(self.linear_conservative.weight, nonlinearity='relu')
    ...
    def training_step(self, train_batch, batch_idx):
        ...
        #self.log('train_loss2', 0.3 * loss2.item())
...
feature_set = 'longformer.pt'

# data
dataset1 = PSPDataset(batch_size=64,
                           name='train', feature_set = feature_set)  # batch_size governs (xxxx / 64) batches of training
dataset2 = PSPDataset(batch_size=1,
                           name='dev', feature_set = feature_set)  # batch_size governs nothing
...
temp = torch.load(feature_set).squeeze(1)

# model
model = PSPDetector(in_channels=len(temp[0]), out_channels=512, dropout=0.5,
                      batch_size=64, num_relations = 5, negative_sample=5, negative_sample_weight=0.1)  # batch_size governs each batch how many nodes
