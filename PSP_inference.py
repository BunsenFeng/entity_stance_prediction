...
class PSPDetector(pl.LightningModule):
    ...
    def test_step(self, val_batch, batch_idx):
        ...
        liberal_prob = self.linear_liberal(node_features)
        conservative_prob = self.linear_conservative(node_features)
        torch.save(liberal_prob, 'inference/liberal_pred.pt')
        torch.save(conservative_prob, 'inference/conservative_pred.pt')
...
checkpoint_path = 'lightning_logs/version_6/'
file = os.listdir(checkpoint_path + 'checkpoints/')[0]
print(file)
model = model.load_from_checkpoint(checkpoint_path = checkpoint_path + 'checkpoints/' + file, hparams_file = checkpoint_path + 'hparams.yaml', map_location = None, in_channels=768, out_channels=512, dropout=0.5,
                      batch_size=64, num_relations = 5, negative_sample=2, negative_sample_weight=0.1)
