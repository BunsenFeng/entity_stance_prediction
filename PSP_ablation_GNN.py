...
class PSPDetector(pl.LightningModule):
    def __init__(self, in_channels, out_channels, dropout, batch_size, num_relations, negative_sample, negative_sample_weight):
        ...
        #self.GCN = GCNConv(self.out_channels, self.out_channels)
        #self.GAT = GATConv(self.out_channels, self.out_channels)
        #self.SAGE = SAGEConv(self.out_channels, self.out_channels)
    ...
    def training_step(self, train_batch, batch_idx):
        ...
        #node_features = self.dropout_layer(self.relu(self.GatedRGCN(node_features, edge_index, edge_type)))
        node_features = self.dropout_layer(self.relu(self.RGCN(node_features, edge_index, edge_type)))
        node_features = self.dropout_layer(self.relu(self.RGCN(node_features, edge_index, edge_type)))
        #node_features = self.dropout_layer(self.relu(self.GCN(node_features, edge_index)))
        #node_features = self.dropout_layer(self.relu(self.GCN(node_features, edge_index)))
        #node_features = self.dropout_layer(self.relu(self.GAT(node_features, edge_index)))
        #node_features = self.dropout_layer(self.relu(self.GAT(node_features, edge_index)))
        #node_features = self.dropout_layer(self.relu(self.SAGE(node_features, edge_index)))
        #node_features = self.dropout_layer(self.relu(self.SAGE(node_features, edge_index)))
    ...
    def validation_step(self, val_batch, batch_idx):
        ...
        #node_features = self.relu(self.GatedRGCN(node_features, edge_index, edge_type))
        node_features = self.relu(self.RGCN(node_features, edge_index, edge_type))
        node_features = self.relu(self.RGCN(node_features, edge_index, edge_type))
        #node_features = self.relu(self.GCN(node_features, edge_index))
        #node_features = self.relu(self.GCN(node_features, edge_index))
        #node_features = self.relu(self.GAT(node_features, edge_index))
        #node_features = self.relu(self.GAT(node_features, edge_index))
        #node_features = self.relu(self.SAGE(node_features, edge_index))
        #node_features = self.relu(self.SAGE(node_features, edge_index))
        liberal_prob = self.linear_liberal(node_features)
        conservative_prob = self.linear_conservative(node_features)
